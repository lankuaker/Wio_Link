...
at_upDate_recon_cb(void *arg, sint8 errType)
{
  struct espconn *pespconn = (struct espconn *)arg;
    ...
    os_free(pespconn);
    at_port_print("disconnect for recon\r\n");
...
at_exeCmdCiupdate(uint8_t id)
{
  ...
  host_ip.addr = ipaddr_addr("192.168.18.146");
  at_port_print("+CIPUPDATE:1\r\n");
  ...
  const char esp_server_ip[4] = {192,168,18,146};
  ...
  {
      at_port_print("+CIPUPDATE:4 started\r\n");
  }
}
#endif
